            ...
            if file.endswith(".ttf"):
                ...
                try:
                    pdfmetrics.registerFont(
                        TTFont(font_name, os.path.join(font_folder, file))
                    )
...
def generate_preview(prefix, start, end, batch_code, mfg_date,
...
preview_count = st.number_input(
    "How many numbers to preview?",
    min_value=1,
    value=3
)

if st.button("Preview Serial Numbers"):
    preview_df = generate_preview(
        prefix, start, end, batch_code, mfg_date,
        preview_start, preview_count
    )
    st.subheader(
        f"Preview of Serial Numbers "
        f"({preview_start} → {preview_start + preview_count - 1}):"
    )
